...
export function getFlagEmoji(code: string): string {
  return String.fromCodePoint(
    ...[...code.toUpperCase()].map((ch) => 0x1f1e6 - 65 + ch.charCodeAt(0)),
  );
}
...
export const COUNTRIES: Country[] = RAW.map(([code, dialCode, name]) => ({
  code,
  dialCode,
  name,
}));

export const PRIORITY_CODES = ['US', 'IN', 'GB', 'CA', 'AU', 'AE', 'SG', 'NZ'];
  ...
  for (const len of [4, 3, 2, 1]) {
    ...
    if (matches.length > 0) {
      const country =
        ...
        matches.find((c) => c.code === 'RU') ??
        matches[0]!;
      return { country, local: digits.slice(len) };
    }
  }
  return null;
}
